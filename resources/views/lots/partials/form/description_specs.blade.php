    <div class="specification_suite_price_item overflow {{ isset($spec) ? 'saved' : '' }}" data-spec-id="{{ isset($spec) ? $spec->id : '' }}" data-suite-spec="{{ isset($key_spec) ? $key_spec : 1 }}">
        ...
        <div class="col l5 m10 s10">
            <div class="input-field spec_value">
                ...
                <input type="hidden" name="spec_price[{{ isset($key_spec) ? $key_spec : 1 }}][spec_desc][{{ isset($key_desc) ? $key_desc : 1 }}][key_unique]" value="{{ isset($spec->key_unique) && !empty($spec->key_unique) ? $spec->key_unique : md5(microtime().rand()) }}"/>
            </div>
        </div>
